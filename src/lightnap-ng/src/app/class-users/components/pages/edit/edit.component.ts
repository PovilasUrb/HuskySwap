...
import { UpdateClassUserRequest } from "src/app/class-users/models/request/update-class-user-request";
...
export class EditComponent implements OnInit {
  ...
  ngOnInit() {
    this.classUser$ = this.#classUserService.getClassUser(this.id()).pipe(
      tap(response => {
        if (response.result) {
  ...
  saveClicked() {
    ...
    this.#classUserService.updateClassUser(this.id(), request).subscribe(response => {
      if (!response.result) {
  ...
  deleteClicked(event: any) {
    ...
    this.#confirmationService.confirm({
      ...
      key: "delete",
      accept: () => {
        this.#classUserService.deleteClassUser(this.id()).subscribe(response => {
          if (!response.result) {
            this.errors = response.errorMessages;
            return;
          }
